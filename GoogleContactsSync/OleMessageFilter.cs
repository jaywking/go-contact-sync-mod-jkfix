...
namespace GoContactSyncMod
{
    ...
    internal interface IOleMessageFilter // Renamed to avoid confusion w/ System.Windows.Forms.IMessageFilter
    {
        [PreserveSig]
    ...
    internal sealed class OleMessageFilter : IOleMessageFilter, IDisposable
    {
        ...
        private void Register()
        {
            ...
            var result = NativeMethods.CoRegisterMessageFilter(this, out _oldFilter);
            if (result != 0)
            {
        ...
        public void Revoke()
        {
            if (_isRegistered)
            {
                var result = NativeMethods.CoRegisterMessageFilter(_oldFilter, out var revokedFilter);
                if (result != 0)
                {
                    throw new COMException("CoRegisterMessageFilter failed", result);
                }
